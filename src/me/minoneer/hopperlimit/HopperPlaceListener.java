	...
	public void onHopperPlace(BlockPlaceEvent e) {
		...
		if (e.getBlock().getType() == Material.HOPPER && !player.hasPermission(HopperLimit.place)) {
			...
			if (plugin.getAndSetCooldown(player.getName(), plugin.config.getCooldownPlace())) {
				player.sendMessage(plugin.config.getCooldownMessage().replaceAll("%cooldown%", String.valueOf(plugin.config.getCooldownPlace())));
				e.setCancelled(true);
				return;
			}
			...
			if (hopperCount > limit) {
				...
				e.setCancelled(true);
				player.sendMessage(plugin.config.getDenyMessage().replaceAll("%limit%", String.valueOf(limit).replaceAll("%radius%", String.valueOf(radius))));
				return;
			}
		}
	}
		...
		if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
			if (e.getClickedBlock() != null) {
				...
				if (type == Material.RAILS || type == Material.ACTIVATOR_RAIL || type == Material.POWERED_RAIL || type == Material.DETECTOR_RAIL) {
					...
					if (e.getItem() != null && e.getItem().getType() == Material.HOPPER_MINECART && !p.hasPermission(HopperLimit.place)) {
						...
						if (hopperCount >= limit) {
							...
							e.setCancelled(true);
							p.sendMessage(plugin.config.getDenyMessage().replaceAll("%limit%", String.valueOf(limit).replaceAll("%radius%", String.valueOf(radius))));
							return;
						}
					}
				}
